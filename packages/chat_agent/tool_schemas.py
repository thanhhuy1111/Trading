...
Pydantic models before any tool handler runs. Anything that fails validation never reaches
`BaselineRecommendationService` / `EvidenceStore`.

Defaults (`BTCUSDT`, `ETHUSDT`, `Timeframe.H1`) match what `packages.research.data_fetcher`
and `packages.research.campaign` use for the campaign's own real-data cache -- this is not a
universe selection decision, just a sane default when the model omits the field.
"""
